...
#                                                         #
# description :                                           #
# generates a h264 container of a dvd for a sony psp      #
###########################################################
...
OUT_TRANS="$HOME/.xbmc/userdata/addon_data/script.video.swiss.army.knife/psp-transcode.log"
...
if [ $# -lt $EXPECTED_ARGS ]; then
  echo "Usage: dvd-psp.sh p1 p2 p3 p4 p5"
  echo "                                      "
  ...
  echo
  echo "dvd-psp..sh was called with wrong arguments"
  echo
  echo example :
  echo
  echo ./dvd-psp.sh /dev/sr0 /dvdrip/portable/psp stargate 1 0 -a 1 -s 0
  echo
...
if [ $# -eq 7 ]; then
   if [[ "$6" =~ ^-a ]] ; then
       ...
       (
        HandBrakeCLI -i $1 -o $2/$3.mp4 -t $4 -A "Audio-1","Audio-2" -a $AUDIO1,$AUDIO2 b 1024 -E faac,faac -B 128 -R 48 -6 dpl2 -f mp4 -X 368 -Y 208 -m &
       ) > $OUT_TRANS 2>&1 &
...
if [ $# -eq 7 ]; then
    if [[ "$6" =~ ^-s ]] ; then
       ...
       if [ "$SHELL_CANCEL" == "0" ] ; then
          ...
          do
                echo -n .
                PASS1=$(strings $OUT_TRANS | grep Encoding | tail -1  | grep "1 of 1" | tail -1 | awk '{print $6}' | cut -d '.' -f1 )
                if [ -n "$PASS1" ] ; then
                   echo $PASS1 > ~/.xbmc/userdata/addon_data/script.video.swiss.army.knife/progress/progress
                   if [ $PASS1 -eq 99 ] ; then
